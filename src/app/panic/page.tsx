    ...
    <main className="max-w-3xl mx-auto p-6 space-y-4">
      ...
      {phase==="intro" && (
        <div className="space-y-3">
          <p className="text-slate-700">You’re safe. We’ll do three short steps together.</p>
      ...
      {phase==="ground" && (
        <div className="space-y-3">
          <p className="text-slate-700">Name: 5 things you can see, 4 you can feel, 3 you can hear, 2 you can smell, 1 you can taste.</p>
          <button className="px-4 py-2 rounded-xl bg-black text-white" onClick={()=>setPhase("done")}>Finish</button>
        </div>
      )}

      {phase==="done" && <div className="text-slate-700">Good job. Come back here anytime.</div>}
    </main>
  );
}
